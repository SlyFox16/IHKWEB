...
<div data-provides="fileinput" class="fileinput fileinput-new input-group">
    <input id="<?php echo $id; ?>" type="file" data-show-cancel="false" data-show-upload="true" data-show-caption="true" name="<?php echo CHtml::activeName($model, $attribute) ?>" data-allowed-file-extensions='["png", "jpg", "jpeg", "gif"]'>
...
<script>
    $(document).on('ready', function(){$("#<?php echo $id; ?>").fileinput({
        <?php if($swf == true && !empty($initial)) { ?>
        initialPreview: "<div class='file-preview-text'>" +
        "<h2><i class='glyphicon glyphicon-file'></i></h2>" +
        "Filename.xlsx" + "</div>",
        <?php } elseif(!empty($initial)) { ?>
        initialPreview: ["<?php echo addslashes(trim($initial))?>"],
        <?php } ?>
